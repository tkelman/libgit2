...
static git_strmap *pack_cache = NULL;
...
int git_mwindow_files_init(void)
{
	...
	return git_strmap_alloc(&pack_cache);
}

void git_mwindow_files_free(void)
{
	git_strmap *tmp = pack_cache;
...
int git_mwindow_get_pack(struct git_pack_file **out, const char *path)
{
	...
	pos = git_strmap_lookup_index(pack_cache, path);
	if (git_strmap_valid_index(pack_cache, pos)) {
		pack = git_strmap_value_at(pack_cache, pos);
	...
	git_strmap_insert(pack_cache, pack->pack_name, pack, error);
	git_mutex_unlock(&git__mwindow_mutex);
...
int git_mwindow_put_pack(struct git_pack_file *pack)
{
	...
	pos = git_strmap_lookup_index(pack_cache, pack->pack_name);
	if (!git_strmap_valid_index(pack_cache, pos)) {
		git_mutex_unlock(&git__mwindow_mutex);
	...
	if (count == 0) {
		git_strmap_delete_at(pack_cache, pos);
